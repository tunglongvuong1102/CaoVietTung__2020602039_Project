...
namespace WebBanHangOnline.Controllers
{
    ...
    public class ShoppingCartController : Controller
    {
        ...
        [AllowAnonymous]
        [HttpPost]
        public ActionResult UpdateDiscount(int discountId)
        {
            // Lấy thông tin mã giảm giá từ cơ sở dữ liệu
            var dis = db.DiscountCodes.FirstOrDefault(d => d.Id == discountId);

            // Gán giá trị giảm giá vào ViewBag
            if (dis != null)
            {
                ViewBag.DisCode = dis.Code;
                ViewBag.DisPer = dis.DiscountPercentage;
            }
            else
            {
                ViewBag.DisCode = "Không có mã giảm giá";
                ViewBag.DisPer = 0;
            }

            // Trả về Partial_Item_ThanhToan dưới dạng HTML
            var partialViewHtml = RenderPartialViewToString("Partial_Item_ThanhToan");
        ...
        [AllowAnonymous]
        public ActionResult Partial_Discount()
        {

            var userId = User.Identity.GetUserId();

            // Lấy danh sách mã giảm giá từ cơ sở dữ liệu
            var discountCodes = db.DiscountCodes
                .Where(d => d.UserId == userId && d.ExpiryDate >= DateTime.Now && !d.IsUsed)
                .ToList();

            // Truyền danh sách mã giảm giá qua model
            return PartialView(discountCodes);
        }
